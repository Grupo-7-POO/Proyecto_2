...
public class UsuariosJson implements IPersistenciaUsuarios
{
     ...
     */
    private void cargarClientes( LearningPath LearningPath, JSONArray jClientes ) throws UserRepetidoException
    {
        ...
        for( int i = 0; i < numClientes; i++ )
        {
            ...
            if( !aerolinea.existeCliente( nuevoCliente.getIdentificador( ) ) )
                ...
                throw new UserRepetidoException( nuevoCliente.getTipoCliente( ), nuevoCliente.getIdentificador( ) );
        }
    }
     ...
     */
    private void salvarClientes( LearningPath LearningPath, JSONObject jobject )
    {
     ...
     */
    private void cargarTiquetes( Aerolinea aerolinea, JSONArray jTiquetes ) throws InformacionInconsistenteUserException
    {
        ...
        for( int i = 0; i < numTiquetes; i++ )
        {
            ...
            if( laRuta == null )
                throw new InformacionInconsistenteUserException( "ruta", codigoRuta );

            String fechaVuelo = tiquete.getString( FECHA );
            Vuelo elVuelo = aerolinea.getVuelo( codigoRuta, fechaVuelo );
            if( elVuelo == null )
                throw new InformacionInconsistenteUserException( "vuelo", codigoRuta + " en " + fechaVuelo );

            String codigoTiquete = tiquete.getString( CODIGO_TIQUETE );
            boolean existe = GeneradorTiquetes.validarTiquete( codigoTiquete );

            if( existe )
                throw new InformacionInconsistenteUserException( "tiquete", codigoTiquete, false );
            ...
            if( elCliente == null )
                throw new InformacionInconsistenteUserException( "cliente", identificadorCliente );
     ...
     */
    private void salvarUsuarios( LearningPath LearningPath, JSONObject jobject )
    {
